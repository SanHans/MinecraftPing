...
class ServerStats:
    ...
    def add_result(self, success: bool, latency: float = 0, version: str = None):
        """添加一次检测结果"""
        if success:
...
class MinecraftPingGUI:
    ...
    def setup_ui(self):
        ...
        columns = ("host", "port", "status", "current", "min", "max", "average", "failures", "version")
        ...
        self.tree.heading("failures", text="失败次数")
        ...
        self.tree.column("failures", width=70)
    ...
    def ping_all_servers(self):
        ...
        for i, (host, port) in enumerate(self.servers):
            ...
            # 更新统计信息
            self.server_stats[server_key].add_result(success, latency, version)
            ...
            if success:
                self.root.after(0, self.update_server_display, i, "在线", 
                              ...
                              stats["average"], stats["failures"], stats["version"])
            else:
    ...
    def update_server_status_only(self, index, status):
        ...
        if index < len(items):
            ...
            new_values = (
                ...
                current_values[7],  # failures
                current_values[8]   # version
            )
            self.tree.item(item, values=new_values)
    
    def update_server_display(self, index, status, current, min_latency, max_latency, average, failures, version):
        """更新服务器显示信息"""
        items = self.tree.get_children()
        if index < len(items):
            item = items[index]
            ...
            new_values = (host, port, status, current, min_latency, max_latency, average, failures, version)
            self.tree.item(item, values=new_values)
